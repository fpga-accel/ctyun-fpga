...
if [ "x$fis_info" == "x" ];then
    echo -e "ERROR:fis tool is not detected, please refer to ctyun-fpga/README.md 2.1.2, go to intall fisclient."
    exit
fi
fischeck_info=`which fischeck 2> /dev/null`
if [ "x$fischeck_info" == "x" ];then
    echo -e "ERROR:fischeck tool is not detected, please refer to ctyun-fpga/README.md 2.1.2, go to intall fisclient."
    exit
fi
...
echo "Important Note:Run the \"fis fpga-image-list --fpga-image-id $shell_id\" command to query the registration status. If the status is \"active\", the registration is complete and the SDK process can be started."
