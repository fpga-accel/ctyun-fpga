...
The SDAccel is stored in the `ctyun-fpga/fp1` directory on a VM by default. Configure the hardware development environment before SDAccel development.

#### Step 1 Configure the EDA tool mode.

Open the **setup.cfg** file in the **ctyun-fpga/fp1/** directory, and set `FPGA_DEVELOP_MODE` to the required FPGA development tool.
